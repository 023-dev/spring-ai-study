...
Spring AI를 활용하면 보다 쉽게 AI 서비스를 만들 수 있지만 실제 서비스에 사용하기에는 여전히 어렵다.
특히 임베딩 모델 선택 및 학습, 벡터 데이터베이스 연결, 리랭킹을 통한 정확도 향상등은 진입 장벽이 꽤 높다.

결국 Spring AI를 활용해 RAG 시스템을 구축하려면, 문서를 청킹하고, 임베딩 모델로 벡터를 생성한 후,
벡터 DB에 저장하고, 질의 시 검색 및 리랭킹 과정을 거쳐야 한다.

그러나 위와 같은 AI의 세부적인 기술들을 모두 알지 못하더라도, AI Agent를 이용해 이를 가능케 할 수 있다.
AI Agent는 특정 도메인 지식에 특화되어 사용자의 질문이나 요청을 스스로 이해하고,
관련 정보를 찾아 보다 정확한 답변을 제공하는 AI이다.
